...
import { sanitizeImageUrl } from '../../lib/safe-url.js';
...
export function renderWishlistListIcon(item) {
  const imageUrl = sanitizeImageUrl(item.imageUrl);
  if (imageUrl) {
    return `
      <span class="wishlist-item-thumb">
        <img src="${escapeHtml(imageUrl)}" alt="" loading="lazy" decoding="async">
      </span>
    `;
  }
  return renderWishlistPriorityIcon(item.priorite);
}
